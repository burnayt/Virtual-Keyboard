...
let leftCards = document.querySelector('.left-cards');
let centerCards = document.querySelector('.center-cards');
let rightCards = document.querySelector('.right-cards');
...
function FillLeftCard(index, name, img){
	console.log(leftCards.children[index]);
	leftCards.children[index].querySelector('p').innerText = name;
	leftCards.children[index].querySelector('img').src = img;
}

function ChangeSliderPets() {
	
	let newPets = pets.filter(function (e) {
		if (lastPets.includes(e.name)) {
			return false;
		}
		return true;
	});
	...
	//let card = CreateCard(newPets[random].name, newPets[random].img)
	
	//leftCards.append(card);
	FillLeftCard(0, newPets[random].name, newPets[random].img);
	lastPets.push(newPets[random].name);
	newPets.splice(random, 1);
	
	random = getRandomInt(newPets.length)
	
	//card = CreateCard(newPets[random].name, newPets[random].img)
	
	//leftCards.append(card);
	FillLeftCard(1, newPets[random].name, newPets[random].img);
	lastPets.push(newPets[random].name);
	newPets.splice(random, 1);
	...
	//leftCards.append(card);
	FillLeftCard(2, newPets[random].name, newPets[random].img);
	lastPets.push(newPets[random].name);
	newPets.splice(random, 1);
}
...
cardContainer.addEventListener('animationend', function(e){
